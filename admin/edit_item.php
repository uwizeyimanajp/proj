...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($errors)) {
        $conn = getDBConnection();
        $stmt = $conn->prepare("UPDATE items SET name = ?, category_id = ?, quantity = ?, price = ?, description = ? WHERE id = ?");
        $stmt->bind_param("siidsi", $name, $category_id, $quantity, $price, $description, $item_id);
...
<head>
    ...
    <title>Edit Item - Vegetable & Fruit Stock Manager</title>
        ...
        <div class="form-container">
            ...
            <form method="POST">
                            ...
                            <div class="category-option">
                                <label>
                                    <input type="radio" name="category_id" value="<?php echo $category['id']; ?>"
                                           <?php echo ($item['category_id'] == $category['id']) ? 'checked' : ''; ?> required>
                ...
                <div class="form-group">
                    ...
                    <input type="number" id="quantity" name="quantity" min="0" value="<?php echo htmlspecialchars($item['quantity']); ?>" required>
                </div>

                <div class="form-group">
                    <label for="price">Price ($):</label>
                    <input type="number" id="price" name="price" min="0" step="0.01" value="<?php echo htmlspecialchars($item['price']); ?>" required>
                </div>
